...
class TransactionRepository {
  ...
  Future<List<Map<String, dynamic>>> getStatDetailed() async {
    ...
    final List<Map<String, dynamic>> result = await db.query("stat_detailed",
  ...
  Future<Map<String, double>> getMonthlyTotals(String yearMonth) async {
    ...
    final List<Map<String, dynamic>> result = await db.query("stat",
      where: 'month = ?',
      whereArgs: [yearMonth],
    );
    ...
    if (result.isNotEmpty) {
      return {
        'income': (result[0]['total_income'] as num?)?.toDouble() ?? 0.0,
        'expense': (result[0]['total_expense'] as num?)?.toDouble() ?? 0.0,
      };
    }
