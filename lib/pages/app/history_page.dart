  ...
  Widget _buildTransactionCard(models.Transaction transaction, models.CategoryModel? category, {bool showRecurring = false}) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(
          (transaction.name == '' ? category?.name : transaction.name) ?? '',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('${transaction.name == '' ? '' : category!.name + ' • '}${DateFormat('yyyy/MM').format(transaction.date)}'),
            if (showRecurring && transaction.recurringMonths != null)
              Text(
                'Every ${transaction.recurringMonths} month(s)${transaction.recurringUntil != null ? ' until ${DateFormat('yyyy/MM').format(transaction.recurringUntil!)}' : ''}',
